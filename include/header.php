<header>
    <div class="menu-sidebar menu-bar-block">
        <div class="className menu-bar-item"><h1>IDM 311</h1></div>
        <a href="../hwk01" class="menu-bar-item menu-button">HW 01</a>
        <!-- <a href="../hwk02" class="menu-bar-item menu-button">HW 02</a>
        <a href="../hwk03" class="menu-bar-item menu-button">HW 03</a>
        <a href="../hwk04" class="menu-bar-item menu-button">HW 04</a>
        <a href="../hwk05" class="menu-bar-item menu-button">HW 05</a>
        <a href="../final" class="menu-bar-item menu-button">Final</a>
        <a href="../caseStudy" class="menu-bar-item menu-button">Case Study</a> -->
        <a href="../test/index.php" class="menu-bar-item menu-button">WebGL Test</a>
    </div>
</header>